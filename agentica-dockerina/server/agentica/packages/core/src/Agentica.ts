...
export class Agentica<Model extends ILlmSchema.Model> {
  ...
  public constructor(private readonly props: IAgenticaProps<Model>) {
    ...
    // STATUS
    console.log("[Agentica.ts] `executor` config status:", typeof props.config?.executor)
    this.executor_
    ...
    const request = async (
      ...
      const event: AgenticaRequestEvent = createRequestEvent({
        ...
        body: {
          ...
          // "Only for Ollama! for OpanAI and other platforms, it'll break"
          ...(this.props.config?.think !== undefined
              ? { think: this.props.config.think }
              : {}),
          
          ...(Object.keys(this.props.config?.options ?? {}).length
